<div align="center">

# 🚀 YAP EN THONG

**`Software Engineer · AI Enthusiast · Robotics Champion`**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/yap-en-thong-325674216/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/enthongy)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:thong04@graduate.utm.my)

</div>

---

## 👾 About Me

```python
class EnThong:
    name       = "Yap En Thong"
    university = "Universiti Teknologi Malaysia (UTM)"
    degree     = "BSc Computer Science (Software Engineering) — Year 3"
    cgpa       = 3.98
    seeking    = "Software Engineering Internship (Sep 2026 – Feb 2027)"
    languages  = ["English", "Malay", "Mandarin"]

    def hello(self):
        print("Building scalable systems, one commit at a time 🚀")
```

---

## 🛠️ Tech Stack

### Languages
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)

### Frameworks & Libraries
![React Native](https://img.shields.io/badge/React_Native-61DAFB?style=flat-square&logo=react&logoColor=black)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white)
![Spring MVC](https://img.shields.io/badge/Spring_MVC-6DB33F?style=flat-square&logo=spring&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

### AI / Computer Vision
![YOLOv8](https://img.shields.io/badge/YOLOv8-00FFFF?style=flat-square&logo=yolo&logoColor=black)
![TensorFlow Lite](https://img.shields.io/badge/TensorFlow_Lite-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)
![EasyOCR](https://img.shields.io/badge/EasyOCR-8A2BE2?style=flat-square&logoColor=white)

### Databases & Tools
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![LabVIEW](https://img.shields.io/badge/LabVIEW-FFDB00?style=flat-square&logo=labview&logoColor=black)
![REST APIs](https://img.shields.io/badge/REST_APIs-009688?style=flat-square&logo=fastapi&logoColor=white)

### 🌱 Currently Exploring
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat-square&logo=flutter&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)

---

## 🔥 Featured Projects

### 🤖 [WIRA.IO](https://github.com/enthongy/wira-io) — Autonomous Fraud Detection Swarm
> Multi-agent fraud orchestration for Malaysian banks, built with Gemini 1.5 Flash, MCP, and Self-Healing ADK for BNM-compliant transaction intelligence.

`TypeScript` `Multi-Agent AI` `MCP` `Gemini` `BNM Compliance`

---

### 🚗 [Parkora.ai](https://github.com/enthongy/parkora-ai) — Smart Traffic & Parking Advisor
> Intelligent city-scale parking and traffic advisory application, designed for smart city infrastructure.

`JavaScript` `AI` `Smart City` `Real-Time Systems`

---

### 📱 [UTM ReMerit](https://github.com/cheng-jiayi/SECJ3104-AD-Section2-GroupHappy) — Campus Sustainability App *(Fullstack)*
> Fullstack mobile app built with a team of 4 to gamify recycling through a reward system — built from DB schema to UI.

`React Native` `Express.js` `MySQL` `REST APIs` `Git`

---

### 😊 [SmileSpace](https://github.com/cheng-jiayi/SECJ3303-IP-Section2-GroupHappy) — Java Application *(Group Project)*
> A collaborative Java project developed as part of UTM's Internet Programming coursework with Group Happy.

`Java` `Internet Programming` `UTM Coursework`

---

### 🔍 [SecureWatch](https://github.com/enthongy/SecureWatch) — License Plate Detection & Recognition
> End-to-end ANPR pipeline using YOLOv8 for detection, EasyOCR for text recognition, and TFLite for edge inference on vehicle type/colour classification.

`YOLOv8` `EasyOCR` `TensorFlow Lite` `Python` `Computer Vision`

---

### 🏆 NIARC 2024 — Autonomous Robotics *(National Champion)*
> Served in the software team, contributing to 1st place among 13 teams from 6 universities. Developed LabVIEW-based autonomous navigation and closed-loop control systems.

`LabVIEW` `Robotics` `Control Systems` `Autonomous Navigation`

---

## 📊 GitHub Stats

<div align="center">

![GitHub Streak](https://streak-stats.demolab.com?user=enthongy&theme=tokyonight&hide_border=true)

</div>

---

## ⚡ Fun Facts

- 🏆 National Robotics Champion — NIARC 2024 (1st place out of 13 teams, 6 universities)
- 🥉 Built *TalkGlove*, a handsign-to-speech app that won Bronze at UTM's Human Communication Interaction Day (42 teams)
- 🏸 Competitive badminton player — Top 4 UTM Games 2025 (Women's Singles), Champion Women Single SUSKOM 2023 & Champion Women's Doubles at UTMKL Sports Festival 2022
- 🎮 Gamer by night — probably debugging code and losing ranked matches at the same time (GENSHIN & PUBG)
- 🎵 Music is the background track to every coding session
- 📖 Anime & reading fan — currently switching between technical papers and something with a good plot twist
- 🪖 Midshipman & Executive Committee at PALAPES UTM (ROTU Navy)
- 🤖 AI Department Lead at AIROST 2024/2025 — UTM's AI, ROS & IoT Club

---

<div align="center">
  <i>Open to Software Engineering Internships — Sep 2026 to Feb 2027</i><br><br>
  <b>Let's build something great together.</b>
</div>
